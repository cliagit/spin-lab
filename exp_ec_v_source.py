...
if conf.getboolean('SOURCE_FIXED'):
    ...
    if conf.getboolean('SOURCE_SQUARE_WAVE'):
        ...
        title = f"{SAMPLE_NAME} voltage from {conf['SOURCE_VAR_MIN_VALUE']} \
to {conf['SOURCE_VAR_MAX_VALUE']}V"
        ...
        try:
            ...
            with open(path_file, "a", encoding='utf-8') as file:
                ...
                if conf.getboolean('SOURCE_FIXED'):
                    ...
                    file.write(f"\nVoltage square waveform source, value \
{conf['SOURCE_SQUARE_VALUE']}V")
                elif SOURCE_FLIPPED:
                    file.write(f"\nVoltage source starts and ends at \
{conf['SOURCE_VAR_MIN_VALUE']}V through {conf['SOURCE_VAR_MAX_VALUE']}V")
                else:
